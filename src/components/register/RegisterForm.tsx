...
import { useRegisterForm, useAuth, Input } from "factoryx-commerce";
...
const RegisterForm = () => {
    ...
    const handleRegister = async (data: RegisterData) => {
        try {
            ...
            console.error('Registration data:', data);
    ...
    const handleOtpSubmit = async () => {
        ...
        try {
            ...
            await signup.mutateAsync(signupData as { Email: string; Password: string });
            
            console.log('Registration with OTP successful!');
            router.push('/login');
        } catch (error) {
            console.error('OTP verification failed:', error);
            alert('OTP doğrulaması başarısız oldu. Lütfen tekrar deneyin.');
        }
    };

    return (
        <form className='flex flex-col mb-16' onSubmit={handleSubmit(handleRegister)}>
            ...
            <div className="flex justify-center lg:mt-auto pt-4 lg:pt-8 border-t-2 border-cool-gray">
                {!showOtpInput ? (
                    ...
                    <button
                        ...
                        disabled={signup.isPending || !otpCode}
                        className="mt-9 w-3/5 h-3/4 bg-green text-white font-semibold hover:bg-green rounded-st uppercase mb-5 disabled:opacity-50"
                    >
                        {signup.isPending ? 'Verifying...' : 'Verify OTP'}
                    </button>
                )}
            </div>
        </form>
