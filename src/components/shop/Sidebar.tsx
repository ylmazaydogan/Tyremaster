...
import { useFilter, useProducts } from "factoryx-commerce";
...
export default function Sidebar() {
    ...
    const brandAttribute = aggregations?.attributes?.find(attr => attr.Name === "Brand");
    // Size filtresi için attributeları bul
    const sizeAttribute = aggregations?.attributes?.find(attr => attr.Name === "Size");
    // Gender filtresi için attributeları bul
    const genderAttribute = aggregations?.attributes?.find(attr => attr.Name === "Gender");
    ...
    return (
        <aside className="w-1/7 bg-white h-full min-h-screen ml-10 mt-20 mb-14">
                        ...
                        {brandAttribute?.Values?.map((brand) => (
                            <label key={brand.key} className="flex items-center">
                                <input
                                    ...
                                    className="mr-2"
                                />
                                <span className="text-sm">{brand.key} ({brand.doc_count})</span>
                            </label>
                        ))}
                    </div>
                </div>
            ...
            {/* Filter By Size */}
            <div className="border-x border-t border-light-silver-2 p-4">
                <h2 className="text-base font-bold text-black uppercase mb-4">Filter by Size</h2>
                    ...
                    {sizeAttribute?.Values?.map((size) => (
                        <label key={size.key} className="flex items-center">
                            <input
                                ...
                                className="mr-2"
                            />
                            <span className="text-sm">{size.key} ({size.doc_count})</span>
                        </label>
                    ))}
                </div>
            </div>
            ...
            {/* Filter By Category */}
            <div className="border-x border-t border-light-silver-2 p-4">
                <h2 className="text-base font-bold text-black uppercase mb-4">Filter by Category</h2>
                <div className="flex flex-col space-y-2 max-h-32 overflow-y-scroll">
                    {aggregations?.categories?.map((category) => (
                        <label key={category.key} className="flex items-center">
                            <input
                                ...
                                className="mr-2"
                            />
                            <span className="text-sm">{category.key} ({category.doc_count})</span>
                        </label>
                    ))}
                </div>
            </div>
            ...
            <div className="border-x border-t border-y border-light-silver-2 p-4">
                <h2 className="text-base font-bold text-black uppercase mb-4">Filter by Gender</h2>
                    ...
                    {genderAttribute?.Values?.map((gender) => (
                        <label key={gender.key} className="flex items-center">
                            <input
                                ...
                                className="mr-2"
                            />
                            <span className="text-sm">{gender.key} ({gender.doc_count})</span>
                        </label>
                    ))}
                </div>
            </div>
